...
const ServicePackageType = () => {
    ...
    const columns = [
        ...
        {
            field: "lookup_value",
            headerName: "Service Package Type",
            headerClassName: "health-table-header-style",
            width: 200,
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData?.subCategoryName?.trim() && formData?.subCategoryName?.length === 0) {
            toast.error("Please enter a valid Service Package Type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_type": "service_package_type",
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Service Package Type updated successfully" : "Service Package Type added successfully");
                setFormData({
                    subCategoryName: '',
                    categoryId: '',
                });
                ...
                fetchData(['service_package_type'], "subCategoryList");
            } else {
                toast.error(res.response.response_message || "Failed to add Service Package Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Service Package Type deleted successfully");
                    fetchData(['service_package_type'], "subCategoryList");
                }
            }
        } catch (error) {
...
<div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Service Package Type."
                description="Add or update the required details for the Service package type to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        ...
                        placeholder="Enter Service Package Type"
