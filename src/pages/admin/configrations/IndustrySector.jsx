...
const IndustrySector = () => {
        ...
        const handleSubmit = async (e) => {
            ...
            if (!brandType?.trim() && brandType?.length === 0) {
                toast.error("Please enter a valid Industry/Sector");
                return;
            }
            setIsLoading(true);
            try {
                const payload = {
                    ...
                    "lookup_type": "industry_sector",
                    "parent_lookup_type": "",
                    "parent_lookup_id": null
                }
                const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
                if (res.response && res.response.response_code === "200") {
                    toast.success(editId ? "Industry/Sector updated successfully" : "Industry/Sector added successfully");
                    setBrandType("");
                    setEditId(null);
                    setIsLoading(false);
                    fetchData(['industry_sector']);
                } else {
                    toast.error(res.response.response_message || "Failed to add Industry/Sector");
                }
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Industry/Sector"
                description="Add or update the required details for the Industry/Sector to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                ...
                <div className="mt-4">
                    <FormButton disable={isLoading}> {isLoading ? (editId ? "Updating..." : "Adding...") : editId ? "Update Industry/Sector" : "Add Industry/Sector"}</FormButton>
                </div>
            </form>
