...
const BloodGroupType = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid Blood Group Type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_value": name,
                "lookup_type": "blood_group_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Blood Group Type updated successfully" : "Blood Group Type added successfully");
                setName("");
                setEditId(null);

                fetchData(['blood_group_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Blood Group Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Blood Group Type"
                description="Add or update the required details for the Blood group type to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        ...
                        name="name"
                        placeholder="Enter Blood Group Type"
                        value={name}
