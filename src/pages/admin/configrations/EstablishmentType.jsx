...
const EstablishmentType = () => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Establishment Type deleted successfully");
                    fetchData(['establishment_type']);
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid Establishment Type");
            return;
        }
        setIsLoading(true);
        try {
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Establishment Type updated successfully" : "Establishment Type added successfully");
                setName("");
                setEditId(null);

                fetchData(['establishment_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Establishment Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Establishment Type"
                description="Add or update the required details for the Establishment Type to keep records accurate and complete."
            />
