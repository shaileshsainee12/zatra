...
const Currency = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Currency updated successfully" : "Currency added successfully");
                setName("");
                setEditId(null);

                fetchData(['currency']);
            } else {
                toast.error(res.response.response_message || "Failed to add Currency");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Currency"
                description="Add or update the required details for the currency to keep records accurate and complete."
            />
