...
const LegalEntityType = () => {
      ...
      const handleEdit = (row) => {
        setLegalEntity(row.lookup_value);
        setEditId(row._id);
      };
        ...
        try {
          ...
          if (result.isConfirmed) {
            ...
            if (res?.response?.response_code === "200") {
              toast.success("Legal Entity Type deleted successfully");
              fetchData(['legal_entity_type']);
            }
          }
        } catch (error) {
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        if (!legalEntity?.trim() && legalEntity?.length === 0) {
          toast.error("Please enter a valid Legal Entity Type");
          return;
        }
        setIsLoading(true);
        try {
          const payload = {
            ...
            "lookup_value": legalEntity,
            "lookup_type": "legal_entity_type",
            "parent_lookup_type": "",
            "parent_lookup_id": null
          }
          const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
          if (res.response && res.response.response_code === "200") {
            toast.success(editId ? "Legal Entity Type updated successfully" : "Legal Entity Type added successfully");
            setLegalEntity("");
            setEditId(null);
            setIsLoading(false);
            fetchData(['legal_entity_type']);
          } else {
            toast.error(res.response.response_message || "Failed to add Legal Entity Type");
          }
 ...
 <div className="p-4 bg-white">
      <SectionHeader
        title="Enter Details for Legal Entity Type"
        description="Add or update the required details for the legal entity type to keep records accurate and complete."
      />
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
        ...
        <div className="mt-4">
          <FormButton disable={isLoading}> {isLoading ? (editId ? "Updating..." : "Adding...") : editId ? "Update" : "Add"}</FormButton>
        </div>
      </form>
