...
const OrganizerType = () => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Organizer type deleted successfully");
    ...
    const handleSubmit = async (e) => {
        ...
        if (!categoryName?.trim() && categoryName?.length === 0) {
            toast.error("Please enter a valid Organizer type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_type": "organizer_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Organizer type updated successfully" : "Organizer type added successfully");
                setCategoryName("");
                setEditId(null);

                fetchData(['organizer_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Organizer type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Organizer type"
                description="Add or update the required details for the Organizer type to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        ...
                        name="categoryName"
                        placeholder="Enter Organizer type"
                        value={categoryName}
