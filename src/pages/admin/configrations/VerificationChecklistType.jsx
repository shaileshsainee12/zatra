...
const VerificationChecklistType = () => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Verification Checklist Type deleted successfully");
                    fetchData(['verification_checklist_type']);
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid Verification Checklist Type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_value": name,
                "lookup_type": "verification_checklist_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Verification Checklist Type updated successfully" : "Verification Checklist Type added successfully");
                setName("");
                setEditId(null);

                fetchData(['verification_checklist_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Verification Checklist Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Verification Checklist Type"
                description="Add or update the required details for the Verification checklist type to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        ...
                        name="name"
                        placeholder="Enter Verification Checklist Type"
                        value={name}
...
export default VerificationChecklistType