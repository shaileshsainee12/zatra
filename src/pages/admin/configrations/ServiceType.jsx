...
const ServiceType = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData?.subCategoryName?.trim() && formData?.subCategoryName?.length === 0) {
            toast.error("Please enter a valid Service Type.");
            return;
        }
        setIsLoading(true);
        try {
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Service Type updated successfully" : "Service Type added successfully");
                setFormData({
                    subCategoryName: '',
                    categoryId: '',
                });
                ...
                fetchData(['service_type'], "subCategoryList");
            } else {
                toast.error(res.response.response_message || "Failed to add Service Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Service Type."
                description="Add or update the required details for the service type to keep records accurate and complete."
            />
