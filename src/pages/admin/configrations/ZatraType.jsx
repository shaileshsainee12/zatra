...
const ZatraType = () => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Zatra Type deleted successfully");
    ...
    const handleSubmit = async (e) => {
        ...
        if (!categoryName?.trim() && categoryName?.length === 0) {
            toast.error("Please enter a valid Zatra Type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_type": "zatra_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Zatra Type updated successfully" : "Zatra Type added successfully");
                setCategoryName("");
                setEditId(null);

                fetchData(['zatra_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Zatra Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Zatra Type"
                description="Add or update the required details for the Zatra type to keep records accurate and complete."
            />
