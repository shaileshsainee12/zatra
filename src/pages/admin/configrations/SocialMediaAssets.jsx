...
const SocialMediaAssets = () => {
    ...
    const columns = [
        ...
        {
            field: "lookup_value",
            headerName: "Social Media Assets",
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Social Media Assets deleted successfully");
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid Social Media Assets");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_value": name,
                "lookup_type": "social_media_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Social Media Assets updated successfully" : "Social Media Assets added successfully");
                setName("");
                setEditId(null);

                fetchData(['social_media_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Social Media Assets");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Social Media Assets"
                description="Add or update the required details for the Social Media Assets to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        ...
                        name="name"
                        placeholder="Enter Social Media Assets"
                        value={name}
