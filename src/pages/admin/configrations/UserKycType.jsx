...
const UserKycType = () => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("User KYC Type deleted successfully");
                    fetchData(['user_kyc_type']);
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid User KYC Type");
            return;
        }
        setIsLoading(true);
        try {
            const payload = {
                ...
                "lookup_value": name,
                "lookup_type": "user_kyc_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "User KYC Type updated successfully" : "User KYC Type added successfully");
                setName("");
                setEditId(null);

                fetchData(['user_kyc_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add User KYC Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for User KYC Type"
                description="Add or update the required details for the User KYC Type to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        label="User KYC Type"
                        name="name"
                        placeholder="Enter User KYC Type"
                        value={name}
