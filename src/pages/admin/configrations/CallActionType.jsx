...
const CallActionType = () => {
    ...
    const columns = [
        ...
        {
            field: "lookup_value",
            headerName: "Call To Action Type",
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Call To Action Type deleted successfully");
                    fetchData(['call_action_type']);
                }
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name?.trim() && name?.length === 0) {
            toast.error("Please enter a valid Call To Action Type");
            return;
        }
        setIsLoading(true);
        try {
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Call To Action Type updated successfully" : "Call To Action Type added successfully");
                setName("");
                setEditId(null);

                fetchData(['call_action_type']);
            } else {
                toast.error(res.response.response_message || "Failed to add Call To Action Type");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Call To Action Type"
                description="Add or update the required details for the call to action type to keep records accurate and complete."
            />
